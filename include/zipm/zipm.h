 ...
 * @param e ZIPM event that just ocurred
 *
 ...
 */
typedef void (*zipm_event_callback_t)(const struct device *dev, int shared_queue_number, void *user_data, enum zipm_event e);
...
/**
 * @brief sends a doorbell signal on the shared queue 
 *
 * @param zdev pointer to the ZIPM device
 * @param shared_queue_number desired shared queue to signal doorbell
 *
 * @return 0 if successful, negative errno code if failure.
 *
 * @note this function just acts a signal raise, it will copy no data
 * to the shared queue desired, it will raise the ZIPM_DOORBELL event
 * and will notify the other side, put in mind that sending a doorbell
 * will overwrite the previous event on the shared queue.
 */
static inline int zipm_send_doorbell(const struct device *zdev, int shared_queue_number)
{
	const struct zipm_device_api *api =
		(const struct zipm_device_api *)zdev->api;

	if (api->send_doorbell == NULL) {
		return -ENOSYS;
	}

	return api->send_doorbell(zdev, shared_queue_number);
}
