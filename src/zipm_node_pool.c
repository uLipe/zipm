...
static inline void zipm_node_pool_lock(const struct zipm_node_pool_header *h)
{
	int key = irq_lock();
...
static inline void zipm_node_pool_unlock(const struct zipm_node_pool_header *h)
{
	int key = irq_lock();
...
uint8_t *zipm_node_pool_alloc(struct zipm_node_pool_header *h)
{
	...
	node = sys_dlist_peek_head(&h->descriptors);
	if(!node) {
...
int zipm_node_pool_dealloc(struct zipm_node_pool_header *h, uint8_t *desc)
{
	...
	zipm_node_pool_lock(h);
	sys_dlist_append(&h->descriptors, node);
	zipm_node_pool_unlock(h);
