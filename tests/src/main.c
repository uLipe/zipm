...
void zipm_event_callback(const struct device *dev, int shared_queue_number, void *user_data, enum zipm_event e)
{
}
...
ZTEST(zipm_test_suite, send_no_memory)
{
	...
	zassert_equal(zipm_send(handle, &data, sizeof(data), 0, 0), -ENOMEM);
	zassert_equal(zipm_flush(handle, 0), 0);
	zassert_equal(zipm_remove_event_callback(handle,&callback), 0);
}
